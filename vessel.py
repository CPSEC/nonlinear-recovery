...
def rudder(command, surge_v):
    rudder_v = 0.166
    rudder_r = 1.661
    rudder_max_angle = deg2rad(30)
    angle = saturate(command, -rudder_max_angle, rudder_max_angle)
    return -rudder_v * surge_v * angle, -rudder_r * surge_v * angle

def propulsion(torque, shaft_speed):
    ...
    max_shaft_speed = 157
    rho = 1000
    this_shaft = (1/Is)*( torque - copysign(1, shaft_speed)*Kq*rho*D**5*(shaft_speed/(2*pi))**2 )
    omega = saturate(shaft_speed, -max_shaft_speed, max_shaft_speed)
    return copysign(1, omega) * Kt * rho * D**4 * (omega/(2*pi))**2, this_shaft

def dcmotor(power, current, omega):
    ...
    I = saturate(current, -max_current, max_current)
...
def whole_model(t, x, u, params={}):
    ...
    power = battery(u[0])
    print(power)
    torque, current = dcmotor(power, x[7], x[6])
    thrust, shaft = propulsion(torque, x[6])
    f_sway, f_yaw = rudder(u[1], x[3])
    first_6 = vessel(x[:6], [thrust, f_sway, f_yaw])
