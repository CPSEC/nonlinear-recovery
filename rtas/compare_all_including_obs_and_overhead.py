...
parser.add_argument('--sim', default='quad_bias', help='cstr_bias, quad_bias, or vessel_bias')
...
for exp in exps:
    ...
    if True:
        ...
        for i in range(0, exp.max_index + 1):
            ...
            exp.model.evolve()
            overhead_dict[bl].append(time.time() - stime)
    ...
    EKF_during_atk = ExtendedKalmanFilter(f=exp.f, jf=exp.jf, jh=exp.jh, dt=exp.dt, Q=exp.StateCov, R=exp.SensorCov) 

    if 'mpc_obs' in baselines:
        bl = 'mpc_obs'
        exp_name = f" {bl} {exp.name} "
        logger.info(f"{exp_name:=^40}")
        for i in range(0, exp.max_index + 1):
            ...
            if exp.recovery_index <= i < recovery_complete_index:
                if (i - exp.recovery_index) % exp.MPC_freq == 0:
                    ...
                    reconstruction_stime = time.time()
                    ...
                    if i == exp.recovery_index:
                        ...
                        k = non_est.get_deadline(x_cur, safe_set_lo, safe_set_up, control, max_k=100, verbose=False)
                        deadline_for_all_methods = k
                        recovery_complete_index = exp.recovery_index + k
                        # logger.debug(f'deadline={k}')
                    # maintainable time compute
                    reconstruction_time = time.time() - reconstruction_stime
                    ...
                    overhead_dict[bl].append(time.time() - stime - reconstruction_time)
    ...
    os.makedirs(f'fig/with_EKF', exist_ok=True)
    EKF_suffix = f'including_obs_{exp.noise_term}disturbance' #+f'_{exp.StateCov[0,0]}QinEKF_{exp.SensorCov[0,0]}RinEKF'
    ...
    # print([overhead_dict[bl] for bl in baselines])
    overhead_dict['nonlinear'] = [0.3, 0.3]
    plt.boxplot([overhead_dict[bl] for bl in baselines+['nonlinear']], labels=baselines+['nonlinear'])
    plt.savefig(f'fig/with_EKF/{exp.name}_overhead_{EKF_suffix}.png', format='png', bbox_inches='tight')